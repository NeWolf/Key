...
public class Keyboard {
    ...
    public static final int VERTICAL_TOP = 0x01;
    public static final int VERTICAL_BOTTOM = 0x02;
    ...
    public static class Key {
         ...
         * that are just out of the boundary of the key. This is a bit mask of
         * {@link Keyboard#VERTICAL_TOP} and {@link Keyboard#VERTICAL_BOTTOM}.
         */
        ...
        public Key(Resources res, Row parent, int x, int y, XmlResourceParser parser) {
            this(parent);
//            Log.e(TAG, "Key: parent = " + parent + " , x = " + x + " , y = " + y );
            this.x = x;
            this.y = y;
            ...
            modifier = a.getBoolean(R.styleable.Keyboard_Key_isModifier, true);
            ...
            verticalFlags = a.getInt(R.styleable.Keyboard_Key_keyVerticalFlags, 0);
...
//        // also recalculate the vertical sizes/positions when we get this resize call.
        ...
        mTotalWidth = newWidth;
//        Log.e(TAG, "resize: mTotalWidth = " + mTotalWidth);
...
//        Log.e(TAG, "getMinWidth: mTotalWidth = " + mTotalWidth);
        return mTotalWidth;
    }
...
//        Log.e(TAG, "computeNearestNeighbors: mCellWidth = " + mCellWidth + " , mCellHeight = " + mCellHeight );
        ...
        for (int i = 0; i < mGridNeighbors.length; i++) {
            int[] cellIndex = mGridNeighbors[i];
            for (int i1 = 0; i1 < cellIndex.length; i1++) {
                bf.append(cellIndex[i1]).append(",");
            }
//            Log.e(TAG, "computeNearestNeighbors: i = "+i +" , bf = " + bf );
            bf.setLength(0);
        }
    }
...
//                        Log.e(TAG, "loadKeyboard: currentRow = " + currentRow );
                        rows.add(currentRow);
                        if (rows.size() == 1) {
                            row0Height = currentRow.defaultHeight;
                        }
...
//                            Log.e(TAG, "loadKeyboard: y = " + y );
                        } else {
                            x += key.gap + key.width;
                            if (row == 0) {
                                y = 0;
                            } else {
                                y = (row - 1) * (currentRow.verticalGap + currentRow.defaultHeight) + currentRow.verticalGap + row0Height;
                            }
                            if (key.edgeFlags == EDGE_RIGHT) {
                                x += key.gap;
                            }

                            if (x > mTotalWidth) {
//                                Log.e(TAG, "loadKeyboard: x > mTotalWidth mTotalWidth = " + mTotalWidth + ", x = " + x);
                                mTotalWidth = x;
                            }
                        }
//
...
//                        }
    ...
    private void parseKeyboardAttributes(Resources res, XmlResourceParser parser) {
        ...
        mDefaultWidth = getDimensionOrFraction(a, R.styleable.Keyboard_keyWidth,
                mDisplayWidth, mDisplayWidth / GRID_WIDTH);
