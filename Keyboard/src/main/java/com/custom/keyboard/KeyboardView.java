...
import com.custom.keyboard.Keyboard.Key;
import com.newolf.keyboard.R;
...
public class KeyboardView extends View implements View.OnClickListener {
    ...
    public static class DefaultOnKeyboardActionListener implements OnKeyboardActionListener {
    ...
    private static final int REPEAT_INTERVAL = 100; // ~20 keys per second
    ...
    public void setBackgroundAlpha(@IntRange(from = 0, to = 255) int alpha){
        ...
        shape.setColor(Color.argb(alpha,5,5,5));
        setBackground(shape);
    }
...
//            Log.e(TAG, "onMeasure: width = " + width + " , paddingLeft = " + paddingLeft + " , paddingRight = " + paddingRight);
...
//        Log.e(TAG, "computeProximityThreshold: mProximityThreshold = " + mProximityThreshold );
    }
...
//            Log.e(TAG, "onBufferDraw: key = " + key );
            ...
            String smallLabel = key.smallLabel == null ? null : adjustCase(key.smallLabel).toString();
...
//            if (key.verticalFlags ==Keyboard.VERTICAL_TOP || key.verticalFlags ==Keyboard.VERTICAL_BOTTOM){
//                Log.e(TAG, "onBufferDraw:verticalFlags  key = " + key );
//                keyBackground.setBounds(0, 0, key.width, key.height/2 );
//            }else{
//                keyBackground.setBounds(0, 0, key.width, key.height);
//            }
            ...
            if (smallLabel != null && label != null) {
//                Log.e(TAG, "onBufferDraw: smallLabel = " + smallLabel + " , label = " + label );
                paint.setTextSize(mLabelTextSize);
                paint.setTypeface(Typeface.DEFAULT);
                // Draw a drop shadow for the text
                paint.setShadowLayer(mShadowRadius, 0, 0, mShadowColor);
                // Draw the text
                float y = (key.height / 2 - padding.top - padding.bottom) / 2
                        + (paint.getTextSize() - paint.descent()) / 2 + padding.top;
                canvas.drawText(smallLabel,
...
//                Log.e(TAG, "onBufferDraw: key.height = " + key.height
...
//                    paint.setTypeface(Typeface.DEFAULT_BOLD);
        ...
        if (DEBUG && mShowTouchPoints) {
            ...
            canvas.drawLine(getWidth() - keys[0].gap, 0, getWidth() - keys[0].gap, getHeight(), paint);
            canvas.drawLine(keys[0].gap, 0, keys[0].gap, getHeight(), paint);
    ...
    private int getKeyIndices(int x, int y, int[] allKeys) {
        ...
        if (DEBUG){
            ...
            for (int i = 0; i < nearestKeyIndices.length; i++) {
                invalidateNearKey(nearestKeyIndices[i],true);
            }
    ...
    private void invalidateNearKey(int keyIndex, boolean isNear) {
    ...
    private void showPreview(int keyIndex) {
        ...
        if (oldKeyIndex != mCurrentKeyIndex) {
            if (oldKeyIndex != NOT_A_KEY && keys.length > oldKeyIndex) {
                ...
                invalidateKey(oldKeyIndex, true);
            }
            if (mCurrentKeyIndex != NOT_A_KEY && keys.length > mCurrentKeyIndex) {
                Key newKey = keys[mCurrentKeyIndex];
                newKey.onPressed();
                invalidateKey(mCurrentKeyIndex, true);
            }
        }
     ...
     */
    public void invalidateKey(int keyIndex, boolean isPressed) {
        if (mKeys == null) return;
        if (keyIndex < 0 || keyIndex >= mKeys.length) {
            return;
        }
...
////        }
//
//        Log.e(TAG, "onTouchEvent: pointerCount = " + pointerCount + " , mOldPointerCount = " + mOldPointerCount );
...
//                    MotionEvent down = MotionEvent.obtain(now, now, MotionEvent.ACTION_DOWN,
//                            me.getX(i), me.getY(i), me.getMetaState());
//                    result = onModifiedTouchEvent(down, false);
//                    down.recycle();
//                }
//
...
//                        MotionEvent down = MotionEvent.obtain(now, now, MotionEvent.ACTION_DOWN,
//                                me.getX(i), me.getY(i), me.getMetaState());
//                        result = onModifiedTouchEvent(down, true);
//                        down.recycle();
//                    }
//                }
//
...
//    }
    ...
    private int[] pressiIndex = new int[12];
...
//            Log.e(TAG, "onTouchEvent: actionIndex = "  + actionIndex + " , id = " + id+ " , index = " + index +" , event = " + event );
        }
...
//                    Log.e(TAG, "onTouchEvent: is not visible , return false" );
                    return false;
                }
                int touchX = (int) event.getX(index) - getPaddingLeft();
                int touchY = (int) event.getY(index) - getPaddingTop();
                ...
                if (keyIndex == NOT_A_KEY) {
//                    Log.e(TAG, "onTouchEvent: keyIndex == 0 id = " + id + " , index = " + index + " , event = " + event);
                    if (DEBUG){
                        for (int i = 0; i < mKeys.length; i++) {
                            if (mKeys[i].isNear){
                                invalidateNearKey(i,false);
                            }
                        }
                    }
                    return false;
                }
                pressiIndex[id] = keyIndex;
                mCurrentKey = keyIndex;
                invalidateKey(keyIndex, true);
                mKeyboardActionListener.onPress(keyIndex != NOT_A_KEY ?
                        mKeys[keyIndex].codes[0] : 0);

                if (keyIndex >= 0 && mKeys[keyIndex].repeatable && mRepeatKeyIndex == NOT_A_KEY) {
            ...
            case MotionEvent.ACTION_UP:
            case MotionEvent.ACTION_POINTER_UP:
                keyIndex = pressiIndex[id];
                invalidateKey(keyIndex, false);
                if (keyIndex >= 0 && mKeys[keyIndex].repeatable) {
                    mRepeatKeyIndex = NOT_A_KEY;
                    mHandler.removeMessages(MSG_REPEAT);
                }
                mKeyboardActionListener.onRelease(keyIndex != NOT_A_KEY ?
                        mKeys[keyIndex].codes[0] : 0);
                if (DEBUG){
                    for (int i = 0; i < mKeys.length; i++) {
                        if (mKeys[i].isNear){
            ...
            case MotionEvent.ACTION_CANCEL:
                ...
                mAbortKey = true;
                showPreview(NOT_A_KEY);
                for (int i = 0; i < pressiIndex.length; i++) {
                    if (pressiIndex[i] != 0) {
                        invalidateKey(i, false);
                    }
                }
    ...
    private boolean onModifiedTouchEvent(MotionEvent me, boolean possiblePoly) {
        ...
        int id = me.getPointerId(actionIndex);
        int touchX = (int) me.getX() - getPaddingLeft();
        int touchY = (int) me.getY() - getPaddingTop();
        Log.e(TAG, "onModifiedTouchEvent: xx actionIndex = " + actionIndex + " , touchX = " + touchX + " , touchY = " + touchY + " , mVerticalCorrection = " + mVerticalCorrection);
        if (touchY >= -mVerticalCorrection)
            touchY += mVerticalCorrection;
        final int action = me.getActionMasked();
        final long eventTime = me.getEventTime();
        int keyIndex = getKeyIndices(touchX, touchY, null);
        Log.e(TAG, "onModifiedTouchEvent: keyIndex = " + keyIndex);
...
//                break;
            ...
            case MotionEvent.ACTION_CANCEL:
                ...
                mAbortKey = true;
                showPreview(NOT_A_KEY);
                invalidateKey(mCurrentKey, false);
                break;
        }
        mLastX = touchX;
        mLastY = touchY;
